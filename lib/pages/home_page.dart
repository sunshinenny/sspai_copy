...
class _HomePageState extends State<HomePage> {
  ...
  void initState() {
    ...
    indexEntityFuture = getHttp(
        "https://sspai.com/api/v1/article/matrix/page/get?limit=60&offset=0");
    print("End initState");
  }
      ...
      builder: (context, snapshot) {
        if (snapshot.hasData) {
          return ListView.builder(
              itemCount: snapshot.data.data.length, // indexEntity.data.length
              itemBuilder: (BuildContext context, int index) {
                return HomeDisplayCard(
                    indexEntityData: snapshot.data.data[index]);
              });
